...
// const LocationSelector = ({onClose, onLocationSet }) => {
...
//       const coordArray = [coordinates.lat, coordinates.lng];

//       if (user) {
//         // Update location for logged-in user
//         const response = await axios.patch(`${API_URL}/auth/user/${user._id}`, {
//           address,
//           coordinates
//         });
...
//         dispatch(
//           updateUserDetails({
//             address: response.data.user.contact.address,
...
//       // Always store location in localStorage
//       localStorage.setItem("userLocation", JSON.stringify(coordArray));
...
//   );
// };
// export default LocationSelector;import React, { useState, useEffect, useRef } from "react";
import { Search, MapPin, Loader2, X } from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import axios from "axios";
import { useNavigate } from "react-router-dom";
...
const LocationSelector = ({ onClose }) => {
  ...
  const handleConfirmLocation = async () => {
    ...
    try {
      ...
      localStorage.setItem("userLocation", JSON.stringify(coordinates));
      console.log("i dont have any user cordinates stored in LS");
      window.location.href = "/home";
    }
    } catch (error) {
      console.error(
